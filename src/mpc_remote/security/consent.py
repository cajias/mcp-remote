...
class ConsentManager:
    ...
    def request_consent(
        self, resource_name: str, is_tool: bool = False, user_context: Optional[Dict[str, Any]] = None
    ) -> ConsentRecord:
        """
        ...
        for record in self._consent_records.values():
            if (
                ...
                and record.is_tool == is_tool
                and record.status in [ConsentStatus.PENDING, ConsentStatus.GRANTED]
            ):
                return record

        # Create new consent record
        record = ConsentRecord(resource_name=resource_name, is_tool=is_tool)
        ...
        if resource_name in self._consent_hooks:
            try:
                consent_granted = self._consent_hooks[resource_name](
                    resource_name=resource_name, is_tool=is_tool, user_context=user_context
                )
                record.status = ConsentStatus.GRANTED if consent_granted else ConsentStatus.DENIED
    ...
    def grant_consent(self, record_id: str, metadata: Optional[Dict[str, Any]] = None) -> ConsentRecord:
        """
    ...
    def revoke_consent(self, record_id: str, reason: Optional[str] = None) -> ConsentRecord:
        """
    ...
    def add_consent_hook(self, resource_name: str, hook: Callable[[str, bool, Optional[Dict[str, Any]]], bool]) -> None:
        """
    ...
    def get_consent_status(self, resource_name: str, is_tool: bool = False) -> ConsentStatus:
        """
        ...
        """
        for record in self._consent_records.values():
            if record.resource_name == resource_name and record.is_tool == is_tool:
...
# Example usage
def example_consent_management() -> None:
    """Demonstrate consent management functionality"""
    consent_manager = ConsentManager()

    # Add a custom consent hook
    def custom_consent_hook(resource_name, is_tool, user_context) -> bool:  # noqa: ANN001
