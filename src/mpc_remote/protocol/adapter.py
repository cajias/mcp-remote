...
class ProtocolAdapter:
    ...
    def adapt_initialize_request(self, params: InitializeParams) -> Tuple[str, Dict[str, Any]]:
        """
    ...
    def adapt_initialize_response(self, protocol_version: str, capabilities: Dict[str, Any]) -> Dict[str, Any]:
        """
        Adapt initialize response from new format to old
        """
        legacy_response = {"protocolVersion": protocol_version, "capabilities": {"streaming": True, "progress": True}}

        # Must include roots capability in response for original SDK
        legacy_response["capabilities"]["roots"] = {"listChanged": True}
    ...
    def _adapt_tool_call(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "arguments" not in params or not isinstance(params["arguments"], dict):
            params = {"name": params.get("name", ""), "arguments": {"args": params.get("arguments", [])}}
