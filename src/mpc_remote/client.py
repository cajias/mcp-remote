...
class MCPClient:
    ...
    async def connect(
        ...
        **kwargs: Any,
        ...
        """
        transport = await cls._create_transport(url, ssl_context, server_key, client_keys, **kwargs)
    ...
    async def _create_transport(
        ...
        **kwargs: Any,
        ...
        if not transport_type:
            raise ValueError(
                f"Unsupported URL scheme: {parsed.scheme}. Supported schemes: {', '.join(cls.TRANSPORT_SCHEMES.keys())}"
            )

        if transport_type == "zmq":
            # Handle secure ZMQ connections
            if server_key:
                return ZMQTransport.create_secure_client(url, server_key, client_keys, **kwargs)
            return ZMQTransport(url, **kwargs)

        elif transport_type == "websocket":
            return WebSocketTransport(url, ssl=ssl_context if parsed.scheme == "wss" else None, **kwargs)
        else:
            return StdIOTransport()
